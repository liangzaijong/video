...
def get_full_model_name(model):
    if model == "gpt-3.5":
        ...
        model = "gpt-4"
    elif model == "ep-20250112204250-tmph4":
        model = "ep-20250112204250-tmph4"
...
def get_llm_kwargs(model, template_version):
    ...
    # Ensure template is a string or a list
    assert isinstance(template, (str, list)), "Template must be a string or a list"
...
def get_layout(prompt, llm_kwargs, suffix="", query_prefix="Caption: ", verbose=False):
    ...
    attempts = 0
    while not done:
        if "gpt" in model or isinstance(template, list):  # Check if template is a list
            r = requests.post(
